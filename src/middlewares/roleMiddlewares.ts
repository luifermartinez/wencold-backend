...
export const isManager = async (
  ...
  if (login.user.role !== userRoles.MANAGER)
    return res.status(403).send({ message: "No estás autorizado." })
  next()
}
...
export const isAdminOrManager = async (
  ...
  if (
    ...
    login.user.role !== userRoles.MANAGER
  )
    return res.status(403).send({ message: "No estás autorizado." })
  next()
}
